...
const FIACAO: { arquivo: string; trecho: string; porque: string }[] = [
  ...
  { arquivo: 'app/api/terapeutas/vendas/editar-paciente/route.ts', trecho: "from('solicitacoes_edicao_paciente')",
    porque: 'sem isto a troca de paciente volta a valer na hora, sem passar pelo CEO - foi essa acao que sobrescreveu uma paciente pela outra em 04/08' },
  { arquivo: 'app/terapeutas/aprovacoes/page.tsx', trecho: 'setEdicoes(',
    porque: 'sem a fila na tela, o pedido de troca fica preso para sempre e o comercial nunca entende por que o nome nao mudou' },
  { arquivo: 'app/api/terapeutas/aprovacoes/edicao-paciente/route.ts', trecho: "from('ocorrencias_prontuario')",
    porque: 'a nota nasce na APROVACAO desde que a troca passou a esperar decisao; sem ela, trocar quem e o paciente volta a nao deixar rastro visivel no prontuario' },
  { arquivo: 'app/api/terapeutas/aprovacoes/edicao-paciente/route.ts', trecho: "from('sessoes')",
    porque: 'sem propagar para as sessoes na aprovacao, o Overview, a agenda e o lembrete seguem com o nome antigo' },
  { arquivo: 'app/api/terapeutas/vendas/editar-paciente/route.ts', trecho: 'mudouIdentidade',
    porque: 'e a guarda que separa "corrigir telefone" de "trocar quem e o paciente"; sem ela a troca volta a valer na hora' },
  { arquivo: 'app/terapeutas/aprovacoes/page.tsx', trecho: 'lancHistorico',
    porque: 'sem isto o lancamento manual SOME da tela depois de decidido e o CEO nao acha mais o que aprovou' },
  ...
  { arquivo: 'app/terapeutas/vendas/page.tsx', trecho: 'terapeutaSugerido(',
    porque: 'sem ela o modal volta a abrir no primeiro da lista (a Denise, por ordem alfabetica) e venda do Pedro cai na agenda dela com a comissao dela' },
  { arquivo: 'app/terapeutas/vendas/page.tsx', trecho: 'avisoTerapeutaDivergente(',
    porque: 'sem o aviso, agendar no terapeuta errado volta a acontecer em silencio' },
