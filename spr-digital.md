...
41. **02/09/2026 - revisao adversarial da branch `feat/oferta-nome` (nome da oferta + pacote pago em duas compras): BLOQUEADO.** A ideia esta certa e a parte pura esta bem feita, mas a ligacao entre as pecas nao fecha e o caminho principal entrega metade das sessoes. Medido no banco real, so leitura, sem nenhuma escrita.

    **O que esta certo e conferido.** `sessoesDoNomeDaOferta` foi rodada contra **os 56 nomes de oferta distintos que existem hoje** em `sales.oferta_nome` (2.311 vendas): 7 nomes devolvem numero e **todos os 7 estao corretos**; os outros 49 devolvem `null` e caem na tabela de preco, como antes. Nenhum caso de quantidade ERRADA no dado real. A tolerancia de R$ 5 esta calibrada num vale verdadeiro: nas 232 vendas de Mentoria particular aprovadas, 202 batem exato, 5 ficam entre R$ 1 e R$ 3, e a proxima esta em **R$ 7,80** - R$ 5 separa os dois grupos e R$ 10 nao separaria. O backfill esta correto: das 2.310 vendas com evento `sale_created`, **2.308 batem com o payload** e as 2 que faltam sao vendas de ontem a noite (21:45 e 21:49), depois do backfill e antes do deploy; nenhum valor errado foi gravado. Os 939 payloads da Hubla tem `products[0].offers[0].name` string em 100% dos casos e os 1.562 da Kiwify em 1.553 (9 sem o campo, que viram `null`). A consulta de Pendentes **nao mudou para venda nenhuma**: `pacote_pai_id` esta nulo nas 10.035 linhas.

    **CRITICO 1 - "E o mesmo pacote" agenda com metade das sessoes.** `app/terapeutas/vendas/page.tsx:622-654` monta as irmas com `vendas_pendentes.concat(vendas_ativos).filter(v => v.pacote_pai_id === agendarVenda.id)`, mas `app/api/terapeutas/vendas/route.ts:276` acabou de **remover de Pendentes justamente as vendas com `pacote_pai_id`**, e elas nao entram em Ativos porque nao tem sessao. A irma nunca volta no payload, `irmas` e sempre `[]`, e a soma de precos que e o coracao da feature (o teste "CASO REAL DA AMANDA") **e codigo morto em producao**. Simulado com os numeros reais dela: antes de responder, `agendarNumSessoes` = 4; depois de ligar as vendas e ate depois de recarregar a pagina, continua 4; so daria 8 se a irma ainda viesse no payload. O comentario em `page.tsx:745-747` afirma o contrario ("senao o agendamento cria 4 sessoes em vez de 8") - o link acontece antes, mas a quantidade ja foi calculada e vai 4 no corpo do POST. Resultado: paciente que pagou R$ 5.280 por 8 sessoes recebe 4, e a segunda compra some de Pendentes para sempre.

    **CRITICO 2 - `pacoteResposta` nunca e resetado e vaza entre vendas.** Declarado em `page.tsx:384` e escrito so pelos dois botoes (1517, 1524). Nao e limpo ao abrir o modal (539-541 e 1150-1153), nem ao fechar (1291, 1616), nem no sucesso (823-825). Depois de responder "E o mesmo pacote" uma vez, **toda venda agendada na mesma sessao de pagina** chama `responderPacote('mesmo_pacote')`: sem candidata, `sale_irma_id` vai `undefined` e a rota devolve 400, travando o agendamento (inclusive o do Diagnostico Guiado, que nao tem nada a ver com a pergunta); com candidata, a tela abre ja com "E o mesmo pacote" pintado e **junta duas compras que ninguem confirmou**.

    **CRITICO 3 - `indeterminado` nao bloqueia, e a quantidade cai para 1 sem ninguem poder corrigir.** O campo editavel de quantidade foi removido. Quando `conferirQuantidade` devolve `indeterminado`, `page.tsx:650-654` cai em `inferirNumeroSessoesPorValor`, que para produto sem preco de tabela devolve **1**, enquanto a tela mostra em vermelho "Nao foi possivel determinar a quantidade... fale com o financeiro antes de agendar" - e o botao Confirmar **continua habilitado** (`page.tsx:1621` nao olha `indeterminado`). No banco: **23 das 232 vendas de Mentoria particular** caem em `indeterminado`, entre elas **6 dos 7 lancamentos manuais recentes** (5 com `preco_base` 0 - Sabrina, Alex, Marcia, Tairine, Adriana, cujos pacotes reais tem 4, 8, 8, 4 e 4 sessoes - e o Kleiton, "Mentoria Particular - Denise Nascimento" a R$ 2.860, pacote real de 4) e o "Restante" do Gabriel Oliveira (R$ 853, pacote real de 2). Sao 34 lancamentos manuais em 90 dias: hoje o comercial digita 8, depois do merge ele nao consegue.

    **CRITICO 4 - a rota `/api/terapeutas/vendas/pacote` confia inteiramente na tela.** `route.ts:62-74` valida apenas que a irma existe e ainda nao tem `pacote_pai_id`. Nao confere **mesmo paciente**, **mesmo produto**, **`sale_irma_id != sale_id`** (venda vira filha de si mesma e some de Pendentes), **status** (venda reembolsada pode virar filha) e, o mais grave, **nao confere se a irma ja tem sessoes**: `candidataAoMesmoPacote` so barra `entregues > 0`, entao uma venda com 8 sessoes `agendada` e proposta normalmente e o resultado e um paciente com dois pacotes ativos. Ciclo tambem passa: ligar A em B e depois B em A e aceito, porque a checagem olha so a irma, e as duas somem de Pendentes.

    **IMPORTANTE.** (a) **Nao existe desfazer**: nenhuma rota ou tela limpa `pacote_pai_id`, e a tela nao mostra em lugar nenhum que uma venda e filha de outra - um clique errado exige cirurgia no banco. (b) **Retry duplica ou trava**: `handleAgendar` chama `responderPacote` antes do agendamento, e o botao "Agendar assim mesmo" do conflito de compromisso (`page.tsx:2148-2150`) chama `handleAgendar` de novo - em `mesmo_pacote` a segunda chamada bate no 409 "Essa outra venda ja faz parte de outro pacote" e o comercial fica **preso**, sem conseguir concluir; em `valor_divergente` e `compra_separada` cada tentativa grava mais uma linha em `ocorrencias_pacote`, mais uma nota no prontuario e mais um `atividades_log`. Pior: a mensagem real do servidor nunca aparece, porque `pacoteErro` e lido no mesmo tick em que foi setado (estado do React ainda velho) e a caixa que o exibiria fica escondida quando `pacoteResposta === 'mesmo_pacote'` (`page.tsx:1549`). (c) **Estado meio-ligado**: o link e commitado antes do insert em `ocorrencias_pacote`; se o insert falhar a rota devolve 500 com as vendas ja ligadas. (d) **As outras duas listas de Pendentes nao conhecem `pacote_pai_id`**: `app/api/terapeutas/dashboard/route.ts:282` continua contando a venda filha como pendente de agendamento, somando sessoes e comissao projetada que nunca serao pagas, e `app/terapeutas/[id]/page.tsx:782` continua listando a filha na aba Pendentes do terapeuta - o botao "Agendar" de la manda para `/terapeutas/vendas?agendar=<id>`, onde a venda foi filtrada, e o comercial recebe "avise o time tecnico". (e) **O caso que originou a feature continua sem solucao**: a venda pendente da Amanda (`dd8f19f1`, R$ 2.600) nao recebe proposta nenhuma, porque a irma (`3690b87a`) ja tem 1 sessao entregue; e abrindo a irma a proposta aparece mas o botao esta travado por reagendamento total com sessao entregue. A resposta so pode ser gravada como efeito colateral de um agendamento que da certo. (f) **O caminho de correcao `-offer-N` da Hubla nao atualiza `oferta_nome`** (`app/api/webhooks/hubla/route.ts:129-136`): nunca disparou em 2.979 eventos, mas quando disparar a linha fica com o nome de oferta gravado pelo formato "simples", que agora decide quantidade de sessao. (g) **Caso real onde a regra nova erra em silencio**: Sylvia Gabriela (`fb2af6a0`), oferta "Formato - 1 Sessao" e `preco_base` 1.300, os dois dizendo 1, e o pacote real no banco tem **2 sessoes**. Situacao `ok`, nenhuma pergunta, e nenhum jeito de corrigir na tela.

    **Os testes nao pegam tudo o que dizem pegar.** 20 mutacoes reintroduzidas uma a uma com `npm test` (arquivos restaurados e md5 conferido no fim): 17 sao pegas, 3 sobrevivem. Duas importam. **Tirar `if (o.id === venda.id) return false`** de `lib/pacote-de-vendas.ts:52` passa com 225/225, e a tela **sempre** passa a propria venda dentro de `outras` - sem a guarda ela viraria candidata de si mesma com distancia zero, ou seja, sempre a escolhida. **Tirar a guarda de pacote misto** (`lib/sessoes-da-oferta.ts:101`, venda sem oferta legivel no meio de um pacote) tambem passa com 225/225. A terceira (`Number.isNaN(quando)`) e mutante equivalente, sem valor. E vale registrar o que **nenhum** teste cobre: `npm test` roda so `lib/*.test.ts`, entao a rota, a ordem das chamadas e todo o estado da tela - onde estao os quatro criticos - podem quebrar com a suite verde.

    **Menores.** O ramo "Sessao Unica" da regex e quadratico (80 mil caracteres levam 1,8 s), so no navegador do comercial; ele tambem falha se a string vier em NFD (falha para `null`, direcao segura). `((offers[0]?.name as string) ?? '').trim()` quebra o webhook com 500 se a plataforma mandar `name` nao-string (nunca aconteceu em 939 payloads, mas e o mesmo padrao ja usado em `produto`). O comentario da migracao `20260903010000` diz que `pacote_pai_id` aponta para "a venda que carrega as sessoes", e o codigo nao garante isso - a rota sempre torna filha a venda passada em `sale_irma_id`. Qualquer usuario ativo, inclusive terapeuta, pode ligar duas vendas quaisquer (`verificarAcesso` nao olha papel, igual as demais rotas operacionais). `diferenca` e `sessoes_do_pacote` sao gravados sem validacao. E `GET /api/terapeutas/vendas` continua sem autenticacao nenhuma (pre-existente, igual em `main`): `oferta_nome` em si nao carrega dado pessoal (os 56 valores sao rotulos comerciais), mas o mesmo endpoint ja devolve nome, e-mail, telefone e valores.

42. **02/09/2026 - re-revisao do commit de correcao `22dcf4b` (os quatro criticos do item 41): BLOQUEADO.** Tres dos quatro fecharam. O primeiro, nao: **o caminho principal continua entregando metade das sessoes**. Medido no banco real, so leitura.

    **CRITICO 1 continua aberto - a soma do pacote acontece uma releitura tarde demais.** A rota passou a devolver `vendas_filhas` (`app/api/terapeutas/vendas/route.ts:287-289`) e a tela passou a ler dali (`app/terapeutas/vendas/page.tsx:643`), com a dependencia certa no `useMemo` (`:649`). O que nao mudou foi **quando** essa lista existe. `vendas_filhas` so e populada por um `loadData()`, e no fluxo real o link e a criacao das sessoes acontecem no MESMO clique: `handleAgendar` chama `responderPacote` em `:804` (que grava `pacote_pai_id` na irma) e logo abaixo, em `:812-834`, faz o POST de `/sessoes/agendar` com `numero_sessoes: agendarNumSessoes` (`:821`) - valor calculado no render ANTERIOR ao link, quando a irma ainda estava em Pendentes e `vendas_filhas` estava vazia. Nao ha `loadData()` entre as duas chamadas, e `agendarNumSessoes` e uma const capturada no closure de `handleAgendar`. Reproduzido com as linhas reais da Amanda lidas do banco: a tela propoe juntar `dd8f19f1` (R$ 2.600) com `3690b87a` (R$ 2.680), `agendarConfere` no instante do clique e `{valor_divergente, sessoes: 4}`, e o corpo do POST vai com **`numero_sessoes: 4`** enquanto o pacote somado vale `{ok, sessoes: 8}`. A rota de agendar nao recalcula nada: `app/api/terapeutas/sessoes/agendar/route.ts:127-129` aceita o numero do corpo. Resultado identico ao do item 41 - paciente que pagou R$ 5.280 recebe 4 sessoes e a segunda compra some de Pendentes. A diferenca e que agora, **depois de recarregar a pagina**, a conta fica certa (8), entao a recuperacao existe mas e destrutiva: reabrir a venda em Ativos e "Apagar e refazer as 4 sessoes", cancelando os convites do paciente. A correcao certa e uma linha: incluir `agendarCandidataPacote` no `conferirQuantidade` quando `pacoteResposta === 'mesmo_pacote'`, para a quantidade enviada ja ser a do pacote inteiro.

    **CRITICO 2 fechado.** O `useEffect` em `:629-635` roda em toda troca de `agendarVendaId`, e todos os caminhos passam por ela: deep link `?agendar=` (`:541`), botao da lista (`:1176`), fechar pelo X (`:1317`), Cancelar (`:1642`) e sucesso (`:849`). Nao sobra `pacoteResposta` sujo entre vendas.

    **CRITICO 3 fechado na trava, aberto na saida.** `indeterminado` vira 0 (`:675-676`) e o botao trava (`:1647` e `:1687`). Mas nao existe mais nenhum caminho para agendar essas vendas: o campo editavel foi removido e nao ha excecao. No escopo atual (123 vendas de Mentoria/Diagnostico dentro do corte do Pedro) sao **7 indeterminado**, sendo 5 lancamentos manuais com `preco_base` 0 e 1 Denise a R$ 2.860. Nenhuma esta em Pendentes hoje (so ha 2 pendentes: o Diagnostico do Francisco, que nao passa por `agendarConfere`, e a Amanda, que e `valor_divergente`), mas `/api/terapeutas/vendas/lancamento-manual` cria a venda SEM sessoes quando `proxima_sessao_data` nao vem (`route.ts:135-139`), e essa venda cai direto no bloqueio. Pior, o bloqueio **impede o proprio fluxo do CRITICO 1**: para ligar duas compras e preciso confirmar o agendamento, e o botao esta travado quando a venda isolada e indeterminada. Nos 6 pares historicos dentro de 24h, 4 sao exatamente assim (Cristiane R$ 4.000 + R$ 1.280 = pacote de 8; Fabio R$ 700 + R$ 700 = 4; Billimaicon; Michelangelo). A rota tambem nao se defende: `numero_sessoes: 0` e falsy e cai em `inferirNumeroSessoes(produto)` (`agendar/route.ts:127-129`), que devolve **1** para "Mentoria Particular - Pedro Roncada".

    **CRITICO 4 fechado no essencial.** Auto-ligacao, ciclo A->B/B->A e corrente de 2 estao barrados; mesmo paciente por e-mail, mesmo produto, irma aprovada e irma sem sessoes estao conferidos na rota. **Sobra a corrente de 3**: a checagem olha se a venda ja e filha, nunca se a irma ja e MAE de alguem, entao com tres compras em 24h (A ligada a B, agendamento falha, depois B ligada a C) fica B->A->C e uma das compras some da soma. Nao ha trio real no banco. **Falso positivo real:** a tela propoe irma com sessoes desde que nenhuma esteja entregue, e a rota recusa qualquer irma com sessao - em 8 propostas historicas simuladas, **2 seriam recusadas** (Amanda e Jessica). E a recusa e invisivel: `handleAgendar` le `pacoteErro` no mesmo tick em que `responderPacote` o setou (`:806`), entao aparece sempre a frase generica, e a caixa que mostraria a mensagem real fica escondida quando a resposta e `mesmo_pacote` (`:1575`). Os dois defeitos ja estavam no item 41 e nao foram tocados.

    **Retry: metade resolvida.** O caminho do "Agendar assim mesmo" nao regrava a resposta (`:804`, `:807`), mas qualquer OUTRA falha do agendamento (conflito com outro paciente, 500, timeout) deixa `pacoteResposta` intacto, e a tentativa seguinte chama `responderPacote` de novo e bate no 409 "Essa outra venda ja faz parte de outro pacote" - com `return`, sem agendar. O comercial fica preso ate recarregar a pagina, sem que nada na tela diga isso. E trocar a resposta para "compra separada" depois de uma falha grava uma segunda `ocorrencias_pacote` contradizendo a primeira. O **estado meio-ligado** tambem continua: o `update` do `pacote_pai_id` e commitado antes do insert em `ocorrencias_pacote` (`pacote/route.ts:104-118`), e um erro no insert devolve 500 com as vendas ja ligadas e nenhum registro para o CEO.

    **Nao endereçados do item 41.** (a) **Nao existe desfazer** nem qualquer indicacao na tela de que uma venda e filha de outra - `pacote_pai_id` so aparece em filtros; um clique errado apaga a venda de todas as listas e exige cirurgia no banco. (g) **Sylvia Gabriela** (`fb2af6a0`, oferta "Formato - 1 Sessao", `preco_base` 1.300, pacote real de 2 sessoes entregues) continua dando `ok` com 1, em silencio e sem correcao possivel na tela. Menores tambem seguem: regex quadratica do ramo "Sessao Unica", `name` nao-string derrubando o webhook com 500, `diferenca`/`sessoes_do_pacote` gravados sem validacao, qualquer usuario ativo podendo ligar duas vendas, e `GET /api/terapeutas/vendas` sem autenticacao.

    **Regressao: limpa.** Para venda sem `pacote_pai_id` os tres filtros novos sao no-op (`!null` e `true`), e Mentoria, Diagnostico, dashboard e tela do terapeuta se comportam igual a `6ae17c2` e a `main`. As colunas novas existem em producao e sao legiveis tanto pela service role quanto pela anon (conferido). `pacote_pai_id` esta nulo nas 10.035 vendas, entao nada mudou de fato ainda. Unica inconsistencia nova: o dashboard tira a filha de pendentes mas `inferirSessoesPorValor` continua dividindo o valor somado pelo numero de irmas, entao a projecao de sessoes/comissao do pacote ligado sai pela metade.

    **Os testes nao cobrem nenhum dos quatro.** `npm test` roda so `lib/*.test.ts`, e as quatro correcoes vivem em `app/`. Provado reintroduzindo cada uma ao contrario: irmas voltando a sair de `vendas_pendentes + vendas_ativos`, `vendas_filhas` removida do payload da rota, `useEffect` do reset apagado, `indeterminado` voltando a 1 com o botao destravado, e a rota de pacote voltando a confiar na tela - **229/229 verdes nas cinco**. As duas mutacoes que o commit diz ter matado morrem mesmo: tirar `if (o.id === venda.id)` quebra 2 testes e tirar a guarda de pacote misto quebra 1. `tsc`, `npm test` (229) e `build` passam.

    **O caso da Amanda continua sem solucao pelo fluxo, e a saida e por dado.** Abrindo a pendente `dd8f19f1` nao ha proposta, porque a irma tem 1 sessao entregue; abrindo a irma `3690b87a` a proposta aparece mas o botao esta travado por reagendamento total com sessao entregue. As 8 sessoes ja estao corretamente montadas em `3690b87a` (`total_sessoes` 8, a primeira entregue em 02/09), entao o unico problema e a outra compra presa em Pendentes. A correcao segura e um `update` de `sales.pacote_pai_id = '3690b87a-806c-47b1-b62b-9a0896a06d6b'` na linha `dd8f19f1`, na direcao que respeita a invariante da migracao (a filha e a que NAO tem sessoes), acompanhado do registro manual em `ocorrencias_pacote` e `ocorrencias_prontuario`. Nenhum numero de faturamento se move: `pacote_pai_id` so e lido pelas tres consultas de Pendentes.

43. **02/09/2026 - revisao de dinheiro dos commits `6732b1d` e `516b71f` (branch `feat/oferta-nome`): BLOQUEADO.** Duas das tres correcoes de dinheiro estao certas e provadas com numero. A terceira - o faturamento por paciente e o reembolso na tela do terapeuta - **nao corrige nada**: carrega as vendas filhas no mapa e nunca as soma. E a escrita direta na Amanda, que era para ser neutra, **moveu numero para pior** em dois lugares. Medido no banco real, so leitura, sem nenhuma escrita. `tsc`, `npm test` (269/269) e `build` passam.

    **A COMISSAO ESTA CERTA.** `app/api/terapeutas/sessoes/agendar/route.ts:142-146` le as filhas (`.eq('pacote_pai_id', sale_id)`) e soma o `valor_liquido` delas antes do `calcularComissao` da linha 150 e muito antes do insert das sessoes da linha 353. A ordem no cliente tambem esta certa: `handleAgendar` (`app/terapeutas/vendas/page.tsx:824-828`) faz `await responderPacote(...)`, que grava o `pacote_pai_id`, e so depois dispara o POST de agendar - a rota le o link ja commitado. **Conferido no contrafactual do Fabio Nery** (`8afe0787` R$ 674,08 + `14b83199` R$ 680,29 liquidos): sem a soma, comissao de R$ 44,06/sessao e **R$ 176,24** no total; com a soma, R$ 88,53/sessao e **R$ 354,10**. E R$ 354,10 e exatamente o que as duas vendas pagam hoje agendadas separadas (2 + 2 sessoes, R$ 88,93 e R$ 88,12 gravados no banco): **juntar o pacote passou a ser neutro no dinheiro do terapeuta**, que e a prova de que a conta fechou. Na Amanda o valor e R$ 0,00 porque o Pedro e socio com 0% (liquido do pacote R$ 5.092,75; se fosse a Denise a 30% seriam R$ 166,44/sessao e R$ 1.331,50 no total, contra R$ 84,48 e R$ 675,84 sem a soma).

    **CRITICO A - o reembolso da tela do terapeuta continua oferecendo R$ 1.380 onde o direito e R$ 3.980.** O commit `6732b1d` acrescentou em `app/terapeutas/[id]/page.tsx:709-713` a consulta que traz as filhas para `vendasMap`, mas **nada consome esse dado**: `p.saleIds` so recebe `sale_id` vindo de `sessoes` (`:1030`), e a filha tem zero sessao por definicao. O bruto e o liquido saem de `p.saleIds.map(sid => vendas[sid])` (`:1038`, `:1049-1050`), entao a filha e carregada e descartada. Simulado com as linhas reais: `/terapeutas/vendas` calcula `valor_pago` = 2.680 + 2.600 = **R$ 5.280** e oferece **R$ 3.980** de reembolso (`app/terapeutas/vendas/page.tsx:734-738`); `/terapeutas/[id]` calcula `valor_pago` = **R$ 2.680** e oferece **R$ 1.380** (`:1171`). **As duas telas discordam em R$ 2.600 no mesmo botao.** Nao e numero de exibicao: `handleReembolso` (`:1338`, `:1350`) grava esse valor em `solicitacoes_reembolso.valor_reembolso`, que e exatamente o que `lib/alertas-reembolso-parcial.ts:80,104` deduz do repasse dos socios quando o CEO aprova - com 35/65, R$ 1.380 no lugar de R$ 3.980 devolve R$ 2.600 a menos ao cliente e deixa R$ 910 a mais com a SPR e R$ 1.690 a mais com o Pedro. Tambem sai errado o card de bruto/liquido por paciente (`:2118`, `:2191`) e o "valor por sessao" do modal (`:2886`: 2.680/8 = R$ 335,00, quando e 5.280/8 = R$ 660,00). **Correcao:** somar as filhas em `p.bruto`/`p.liquido`, por exemplo acrescentando `Object.values(vendas).filter(v => p.saleIds.includes(v.pacote_pai_id ?? ''))` a `vendasDoPaciente` em `:1038`.

    **CRITICO B - a escrita direta na Amanda apagou R$ 2.600 do faturamento do terapeuta.** O item 42 afirmou que "nenhum numero de faturamento se move" ao ligar `dd8f19f1` em `3690b87a`. Move. Antes do link a filha era pendente do Pedro e entrava em `pendentesT.bruto` (`app/api/terapeutas/dashboard/route.ts:301`); depois do link ela sai de pendentes e **nao entra em lugar nenhum**, porque `fat_bruto_t` (`:375-377`) so olha `saleIdsTerapeuta`, que vem das sessoes. Resultado: a linha do Pedro na tabela de `/terapeutas` (`app/terapeutas/page.tsx:358`) caiu de R$ 5.280 para **R$ 2.680** nessa paciente, enquanto o card global de Faturamento bruto da mesma pagina (`:204`) continua com os R$ 5.280, porque ele soma `vendasFaturamento` sem filtro de pendente. **Duas caixas na mesma tela, R$ 2.600 de diferenca.** O que melhorou no mesmo movimento: as sessoes projetadas do Pedro para a Amanda cairam de 12 (8 reais + 4 projetadas) para 8, que e o numero certo. Nada no DRE, no fechamento dos socios, no caixa ou no `getSales()` se moveu - `pacote_pai_id` e `oferta_nome` nao sao lidos por nenhum arquivo do lado financeiro (conferido por varredura em `app/dre`, `app/fechamentos`, `app/caixa`, `app/vendas`, `app/api/sales`, `app/api/closings`, `app/api/cashflow`, `lib/services.ts`).

    **CRITICO C - "Separar" devolve a compra para Pendentes e deixa a comissao dobrada, sem aviso.** O `DELETE /api/terapeutas/vendas/pacote` (`route.ts:220-222`) zera o `pacote_pai_id` e nao toca em `sessoes`. O modal avisa que "as sessoes ja criadas nao sao alteradas" (`app/terapeutas/vendas/page.tsx:2175`), mas nao diz o que isso custa: a venda-pai fica com as sessoes dimensionadas para o pacote INTEIRO, cada uma com a comissao calculada sobre a soma das duas compras, e a filha volta a Pendentes para ser agendada de novo com sessoes e comissao proprias. Num par de R$ 700 + R$ 700 da Denise: a pai mantem 4 sessoes a R$ 88,53 (R$ 354,10) e a filha gera mais 2 a R$ 88,93 (R$ 177,86) - **R$ 531,96 de comissao sobre R$ 1.354,37 de liquido, 39% em vez de 30%** - e o paciente fica com 6 sessoes tendo pago 4. Nao ha como consertar pela tela quando alguma sessao ja foi entregue: `planejarReagendamentoTotal` (`lib/reagendamento-total.ts:79-86`) recusa o reagendamento total nesse caso, e `sessoes.comissao_valor` so e escrito no insert (conferido: nenhuma outra rota escreve nessa coluna).

    **A comissao ja gravada nunca e recalculada, e nao ha caminho para isso.** `sessoes.comissao_valor` e escrito uma unica vez, em `agendar/route.ts:342,350` e em `lancamento-manual/route.ts:104`. `POST /api/terapeutas/vendas/pacote` e o `DELETE` so mexem em `sales.pacote_pai_id`. O fechamento do terapeuta (`app/api/terapeutas/fechamentos/route.ts:21,27,43,136`) soma `comissao_valor` como esta e nao conhece `pacote_pai_id` nem `oferta_nome`. **Hoje isso custa R$ 0,00**: existe 1 unico `pacote_pai_id` preenchido em 10.037 vendas, sao 8 sessoes, todas do Pedro a 0%, e recalculadas com o pacote continuam em R$ 0,00. Com a Denise a 30% as mesmas 8 sessoes valeriam R$ 655,66 a mais. **Deveria recalcular** para as sessoes ainda com `comissao_paga = false` e nao entregues, que na Amanda sao todas as 8.

    **IMPORTANTE - a soma das filhas nao filtra status.** `agendar/route.ts:142-143` soma o `valor_liquido` de toda linha com `pacote_pai_id = sale_id`, inclusive filha `reembolsada` ou `chargeback`. A tela, do outro lado, deixa de ver a filha estornada (`classificarVendas` monta `filhas` a partir de `aprovadas`, `lib/vendas-por-situacao.ts:77`), entao num reagendamento da pai a tela conta 4 sessoes e a rota paga comissao sobre as duas compras: com a Denise, R$ 332,87/sessao no lugar de R$ 168,96, **R$ 655,64 a mais** sobre dinheiro que voltou para o cliente. Basta acrescentar `.eq('status','aprovada')` na consulta.

    **IMPORTANTE - estorno de uma venda do pacote nao mexe em sessao nenhuma.** O webhook da Hubla (`app/api/webhooks/hubla/route.ts:236-244`) so atualiza `status` e `data_reembolso`. Com a filha estornada, o paciente fica com as 8 sessoes da pai tendo pago 4, a comissao segue calculada sobre o pacote inteiro, e o banner "Pacote pago em mais de uma compra" da tela some (ele filtra `vendas_filhas`, que exige `aprovada`) - **junto com o botao "Separar", unico caminho para desfazer o link**. Com a PAI estornada e pior: a filha continua `aprovada` com `pacote_pai_id` preenchido, fica fora de Pendentes para sempre, o banner nunca renderiza porque a pai saiu de Ativos, e R$ 2.600 de dinheiro real ficam sem sessao e sem nenhuma tela. A deducao dos socios funciona nos dois casos (`lib/alertas-reembolso.ts:83` deduz o `valor_liquido` da venda estornada, uma vez so), mas nao ha deducao equivalente para a comissao do terapeuta.

    **IMPORTANTE - a rota de agendar nao se defende de agendar uma venda que ja e filha.** O `select` da linha 56-57 nao traz `pacote_pai_id` e nao ha guarda nenhuma. Hoje as tres telas filtram a filha, entao o unico caminho e uma pagina aberta antes do link: dois comerciais, um liga e agenda, o outro ainda ve a filha em Pendentes e agenda tambem - **8 sessoes na pai + 4 na filha, 12 para um pacote de 8**, com comissao sobre o liquido das duas somado mais o liquido da filha de novo. Uma linha resolve: recusar quando `sale.pacote_pai_id` estiver preenchido.

    **IMPORTANTE - a janela de 24h nao existe na rota.** `avaliarLigacao` (`lib/ligacao-de-pacote.ts:51-93`) confere e-mail, produto, status da irma, ciclo, corrente e sessoes da irma, mas **nao confere a distancia entre as duas compras**, que e a regra central do negocio (`JANELA_MESMO_PACOTE_HORAS`, `lib/pacote-de-vendas.ts:21`) e o que separa "pagamento dividido" de "cliente comprando de novo". Ela vive so no cliente, em `candidataAoMesmoPacote`. A rota tambem nao le o `status` da propria venda-pai, entao uma venda reembolsada pode receber filha. A rota declara no comentario que "NAO confia na tela"; nesses dois pontos ela confia.

    **IMPORTANTE - a projecao do dashboard continua dividindo pelo numero de irmas (item 42, ainda aberto).** `inferirSessoesPorValor` (`dashboard/route.ts:265-272`) soma o `preco_base` das vendas do mesmo e-mail e produto e divide pelo numero de irmas. Com o pacote ligado a filha saiu de Pendentes mas continua no denominador: para a Amanda, soma 5.280 -> 8 sessoes, dividido por 2 irmas = **4**. Enquanto a pai nao for agendada com sucesso (link commitado e agendamento recusado por conflito, timeout ou desistencia), o dashboard projeta 4 sessoes onde o pacote tem 8 e comissao sobre o liquido de uma compra so. A conta certa e somar o `preco_base` das filhas na venda-pai e nao dividir.

    **Menores.** `SaleRow` do dashboard (`route.ts:7-18`) nao declara `pacote_pai_id`, entao `ehPendenteDeAgendamento` compila mesmo se alguem tirar a coluna de `COLUNAS_DO_DASHBOARD` - a regra passaria a valer sempre `true` sem erro nenhum (o teste "o select do dashboard carrega pacote_pai_id" cobre a constante, nao o tipo). O retry de agendamento que falha por qualquer motivo que nao seja conflito de compromisso regrava a resposta e insere uma segunda linha em `ocorrencias_pacote`, uma segunda nota de prontuario e um segundo `atividades_log` (a rota devolve `ja_ligada`, entao o link nao duplica, so a auditoria). E a filha nao e alcancavel pelo prontuario enquanto esta `aprovada`: `prontuarioSale` procura em `vendas_pendentes + vendas_ativos + vendas_reembolsos` (`app/terapeutas/vendas/page.tsx:707-709`), nunca em `vendas_filhas`.

    **O que ficou provado certo.** A comissao do pacote ligado (numero e momento). O reembolso na tela do comercial (R$ 3.980 na Amanda, com os R$ 5.280 corretos). A saida da filha de Pendentes nas tres listas (`lib/vendas-por-situacao.ts` usada pela rota do comercial, pelo dashboard e pela tela do terapeuta). A conferencia do CEO em `ocorrencias_pacote` (o `GET` novo). O DRE e o fechamento dos socios, que continuam contando as duas compras como duas linhas de receita - **e devem continuar**: cada compra e dinheiro que entrou de verdade, e juntar as duas numa linha so faria o faturamento historico encolher e deixaria de bater com o painel da plataforma.

44. **03/09/2026 - a rodada de correcao das cinco revisoes (A, B, C, D, E) na branch `feat/oferta-nome`.** Tres revisoes bloquearam (A, B, C), uma mediu cobertura (D) e uma auditou o dado em producao (E, banco integro, zero criticos). Todos os achados de dinheiro e de dado escondido foram corrigidos. `tsc` limpo, `npm test` 313/313, `next build` OK.

    **A soma do pacote virou um modulo so: `lib/dinheiro-do-pacote.ts`.** A MESMA soma estava escrita a mao em tres lugares - a comissao na rota de agendamento, o reembolso no prontuario do comercial e o faturamento por paciente na tela do terapeuta - e os tres discordavam. `liquidoDoPacote` (base da comissao), `brutoDoPacote` (base do reembolso) e `saleIdsComAsFilhas` (a lista de vendas de um paciente) tem 9 testes com numero real do banco. Duas guardas dentro dele que parecem redundantes e nao sao: `f.id !== pai.id`, porque trocar `.eq('pacote_pai_id', x)` por `.eq('id', x)` devolve a propria venda-pai dentro da lista de filhas e paga a comissao em dobro sem erro em tela nenhuma; e o filtro de status, porque uma filha reembolsada sumia da tela e continuava somando na comissao.

    **CRITICO A (revisao A e C) - o reembolso da tela do terapeuta oferecia R$ 1.380 onde o direito e R$ 3.980.** A correcao anterior carregava as filhas em `vendasMap` e nunca as somava: `p.saleIds` nasce das SESSOES, e a filha, por definicao, nao tem nenhuma. Corrigido com `saleIdsComAsFilhas` em `app/terapeutas/[id]/page.tsx`. As duas telas gravam na MESMA fila de `solicitacoes_reembolso` e o CEO aprova o numero que vier; com 35/65, a diferenca de R$ 2.600 saia do bolso do cliente e ficava com a SPR (R$ 910) e com o Pedro (R$ 1.690).

    **CRITICO B (revisao B) - as filhas eram filtradas pelo periodo da tela.** `vendas_filhas` saia de `vendasAll`, que passa por `.gte('data_hora', from)`. A janela da regra e de 24h, entao o par quase sempre atravessa a meia-noite: com o preset "Hoje" em 25/08 o pai entrava, a filha nao, e o comercial agendava 4 sessoes num pacote de 8 - exatamente o defeito que a feature existe para impedir. Agora as filhas sao buscadas por `.in('pacote_pai_id', ...)` FORA do filtro de data e do corte, em lotes de 200.

    **CRITICO C (revisao A) - o faturamento do terapeuta perdia R$ 2.600.** Depois de ligada, a filha saia de Pendentes (certo) e nao entrava em lugar nenhum: `fat_bruto_t` so olhava `saleIdsTerapeuta`, que vem das sessoes. A linha do Pedro mostrava R$ 2.680 e o card global da MESMA pagina mostrava R$ 5.280. Corrigido com `saleIdsComAsFilhas` tambem no dashboard.

    **CRITICO D (revisao C) - trocar a resposta nao desfazia a ligacao.** Sequencia toda por botao: o comercial junta as compras, o agendamento falha (conflito de horario, 500, timeout), ele reconsidera e clica em "E compra separada". `avaliarLigacao` devolvia `so_registrar` na primeira linha: **a ligacao continuava de pe**, gravava-se so uma segunda ocorrencia dizendo o contrario da primeira, e como `handleAgendar` nao recarregava, a soma voltava a 4 sessoes. Agora existe a acao `desligar`, com rollback simetrico se a auditoria falhar (`refazerLinkSeAuditoriaFalhar`), e a tela recarrega mesmo tendo falhado. A ocorrencia carrega a marca do desfazer, para o CEO nao ler "Compras separadas" onde houve uma retratacao.

    **CRITICO E (revisao A e C) - "Separar" devolvia a compra e deixava a comissao dobrada.** O `DELETE` nunca olhava se a venda-pai tem sessoes. Com o pacote ja agendado, o pai ficava com 8 sessoes sustentadas por uma compra que vale 4 e a filha voltava a Pendentes com o botao "Agendar" ativo: paciente com 8 + 4 = 12 sessoes tendo pago 8, e no par real do Fabio Nery (Denise a 30%) R$ 531,15 de comissao no lugar de R$ 354,10. **A rota agora recusa com 409 quando o pai tem sessoes**, dizendo quantas e o que fazer antes. O modal, que dizia "As sessoes ja criadas nao sao alteradas" - tranquilizador e exatamente o oposto do que importa -, agora diz a verdade.

    **O "Separar" so funcionava onde nao era alcancavel.** Com a guarda acima, ele vale so quando o pai nao tem sessoes; e era justo nesse estado que o botao nao existia, porque o chip e o botao moravam no prontuario e o prontuario so abre na sub-aba Ativos. Pendentes ganhou o aviso "+ R$ X juntados" e o proprio botao.

    **A janela de 24h passou a valer no servidor.** `avaliarLigacao` conferia e-mail, produto, status, ciclo e corrente, mas nao a distancia entre as compras - a regra central do negocio, que vivia so no cliente. A rota aceitava juntar duas compras separadas por meses, que e exatamente o caso que a regra existe para nao juntar. Sem as duas datas ela recusa: assumir que passa juntaria compras distantes sem ninguem ver.

    **A rota de agendar passou a recusar uma venda que ja e filha.** Duas abas abertas em paralelo agendavam o pai e a filha: 12 sessoes num pacote de 8, com o liquido da filha contado duas vezes. A tela ja escondia o botao; a rota agora tambem recusa (409).
    ...
    **A filha estornada parou de sumir do pacote.** `filhas` saia de `aprovadas`; agora sai de `vendas`. Chargeback de uma das duas parcelas e justamente o motivo pelo qual alguem paga em duas compras, e o pai perdia o aviso, o botao de separar e o valor sem nada dizer. Quem decide se ela vale DINHEIRO e `filhasDoPacote`, nao a lista.

    **A projecao do dashboard parou de dividir pelo numero de irmas.** `inferirSessoesPorValor` somava R$ 5.280, achava 8 sessoes na tabela e dividia por 2 irmas, projetando 4. Agora o NOME DA OFERTA vem antes de qualquer chute por valor, nas duas telas que faziam essa conta. A quantidade e regra da empresa e esta escrita na oferta vendida.

    **A segunda tentativa parou de contar a irma duas vezes.** Efeito colateral do recarregamento acima: a candidata passava a chegar TAMBEM por `filhas`, e a conta somava 4 + 4 + 4 = 12 sessoes. E a resposta parou de ser regravada: "e o mesmo pacote" sem candidata nao tem o que gravar, entao acabaram a segunda `ocorrencias_pacote`, a segunda nota de prontuario e a segunda linha de log dizendo a mesma coisa. O campo `ehRetryDeCompromisso`, que existia e nunca era preenchido, foi ligado - a decisao duplicada no `handleAgendar` saiu.

    **A tela de Aprovacoes.** O fetch da conferencia estava dentro do `try` que controla o `loading`, sem timeout: um fetch pendurado (Vercel lenta, aba voltando do sono) deixava a tela dos reembolsos eternamente no spinner, com os dados ja carregados e invisiveis. Agora ele roda depois do `setLoading(false)`, com `AbortController` de 15s, e a falha aparece em tela com botao de tentar de novo - numa tela cuja funcao e conferir, silencio e a pior resposta. O rotulo e o texto da diferenca sairam do JSX para `lib/conferencia-de-pacote.ts`: o sinal invertido em "Faltaram/Entraram" e o tipo de erro que ninguem revisa no meio do JSX.

    **As tres telas passaram a concordar sobre o Diagnostico Guiado.** Elas escolhiam de quem e a venda por "o produto contem o primeiro nome do terapeuta". "Diagnostico Guiado: Programa de acompanhamento Individual" nao contem nem "pedro" nem "denise": a tela do comercial abria excecao por termo fixo e mostrava, o dashboard e a tela do terapeuta nao. Agora e `ehDoTerapeuta`, em `lib/vendas-por-situacao.ts`. Media 1 venda hoje: Francisco Geraldo, R$ 4.997.

    **O `GET /api/terapeutas/vendas/pacote` ganhou porteiro.** Ele devolvia nome do paciente, produto e a `justificativa` de texto livre que o comercial digita sobre ele - o campo mais sensivel do modulo - sem nenhuma checagem, so a URL. A checagem e por usuario ATIVO, nao por senha nem token, e isso e escolha consciente: a tela de Aprovacoes guarda so `{nome, email, tipo}` em `localStorage`, e o caminho por token so passa para quem tem `dispensa_senha_nas_acoes`. **Protege varredura anonima; nao protege quem saiba um e-mail cadastrado.** As rotas `GET` de `/vendas` e `/aprovacoes` tem a mesma lacuna e continuam abertas - autenticacao de GET neste modulo e item pendente. O `select('*')` virou lista de colunas nomeadas.

    **A medicao por mutacao da revisao D, e o que ela obrigou a fazer.** 12 defeitos desta feature foram reintroduzidos um a um em `app/`: **os 12 passaram verdes**, com `tsc` limpo nos 12. Os controles em `lib/` morreram como deviam (3, 1 e 1 falhas), provando o metodo. O achado que decidiu a resposta: depois de extrair a decisao para modulo puro, **mutar o call-site ja refatorado continuou verde**. Extrair para `lib/` mata mutacao DENTRO da decisao; nao mata "alguem apagou a chamada", porque `npm test` roda so `tsx --test lib/*.test.ts` e rota e componente nunca sao executados. Dai `lib/fiacao-do-pacote.test.ts`: 17 asseroes que so verificam que a chamada continua no arquivo. Ele NAO prova que o numero esta certo - quem prova e `dinheiro-do-pacote.test.ts` -, dispara em refatoracao legitima (e por isso a lista e curta e cada linha traz o porque), e e o unico teste que pega apagamento enquanto nao houver runner de rota e de componente. Os dois `SaleRow` ganharam `pacote_pai_id` e `oferta_nome`: sem a coluna no tipo, apagar o campo do `select` nao da erro de compilacao nenhum, o valor chega `undefined` e a regra apenas para de valer.

    **A auditoria de producao (revisao E) nao achou nada corrompido.** 10.038 vendas, 0 com `oferta_nome` sem respaldo no payload do proprio `order_id`, 0 `order_id` duplicado, 0 evento orfao no Google Calendar, 0 sessao apontando para evento inexistente, 0 residuo sintetico das intervencoes de hoje. Os 17 orfaos apagados eram 17 exatos, nenhum futuro. Os dois fluxos do n8n seguem ativos e a alteracao e aditiva: das 129 sessoes futuras, 44 ganham a etiqueta e 85 recebem a mensagem byte a byte igual a de antes.

45. **03/09/2026 - os pacientes do Diagnostico Guiado nao apareciam no Overview do terapeuta.** Relatado pelo usuario. Confirmado e corrigido no mesmo dia.

    **A causa.** `GET /api/terapeutas/dashboard` varre `sales` filtrando por "o produto contem o nome de algum terapeuta" (`nomesTerapeutas.map(n => produto.ilike.%${n}%)`). O produto do Diagnostico e "Diagnostico Guiado: Programa de acompanhamento Individual" e **nao contem nem "pedro" nem "denise"**, entao as 8 vendas dele nunca entravam em `vendasRaw`. Como `consultas_hoje` e `proximas_consultas` sao filtradas por `.in('sale_id', saleIds)` e o `saleIds` sai dessa varredura, **as sessoes do Diagnostico sumiam do Overview inteiro**. A tela do comercial ja abria excecao com um termo fixo; o dashboard nao. Duas varreduras, a mesma pergunta, respostas diferentes.

    **Medido antes e depois, contra o banco de producao.** Antes: 5 consultas hoje, 77 proximas, **0 do Diagnostico**. Depois: 7 hoje, 86 proximas, **11 sessoes do Diagnostico de 7 pacientes** - Paula Caroline, Bruno Cavallini, Gisela Palos, Jeane Gomes, Rafaela Pires, Valdir Sabino e Francisco Geraldo.

    **A correcao.** O termo virou `TERMO_SQL_DIAGNOSTICO` e a montagem virou `termosDeProduto(nomesTerapeutas)`, em `lib/vendas-por-situacao.ts`, usada pelas DUAS varreduras. Teste proprio, mais duas linhas em `lib/fiacao-do-pacote.test.ts` (uma por rota): as duas varreduras ja divergiram uma vez e ninguem viu.
