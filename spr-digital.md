...
**PENDENTE, nao bloqueia a feature em si:** a migration
`supabase/migrations/20260901000000_atividades_log_empurrar_seguintes.sql`
(acrescenta `'empurrar_seguintes'` à allow-list de
`atividades_log_tipo_acao_check`) **não foi aplicada em produção** - o sandbox
de implementação não tem permissão para DDL (`supabase db push`/`migration up
--include-all` foram negados pelo classificador de modo automático). Até
alguém com permissão de escrita de schema rodar
`supabase migration up --linked --include-all` (as outras 4 migrations que
apareceriam junto já estão aplicadas de fato na base, só faltam no histórico do
CLI, e são idempotentes), toda chamada a `/empurrar-seguintes` continua
movendo as sessões normalmente, mas o insert em `atividades_log` falha com
`23514` e nenhum rastro fica gravado - terceiro caso do mesmo defeito no
projeto, ver item 36.
