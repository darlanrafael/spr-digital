...
- **Painel do próprio terapeuta (`/terapeutas/[id]`, redesenhado em 05-06/07/2026):** quando quem loga é o próprio terapeuta (sessão `terapeutas_session` com `tipo === 'terapeuta'`), a tela mostra três abas próprias — **Overview**, **Vendas** e **Fechamentos** — espelhando as telas que o CEO já usa (`/terapeutas` e `/terapeutas/vendas`), só que escopadas para este terapeuta:
